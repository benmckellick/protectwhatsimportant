...
b>a&&(a=b)});$("#canvas .page").hide();c.show();c=$canvas.height();600<c&&($(".container").css({"overflow-y":"auto",height:parseInt(c,10)+250+"px"}),supercanvas.css("height","100%"));if(!$("#canvas #grid-div").length)switch($("#canvas").prepend($(".grids").show().contents().unwrap()),$("#show-grid").attr("checked",!0),$canvaswrap.attr("data-grid")){case "graph":$("#grid-div").show();$("#gridcol , #grid-dot").hide();$("select#grid-type").val("graph");break;case "dots":$("#grid-dot").show();$("#gridcol , #grid-div").hide();
$("select#grid-type").val("dots");break;case "col":$("#gridcol").show();$("#grid-div , #grid-dot").hide();$("select#grid-type").val("col");break;default:$("#grid-div , #grid-dot , #gridcol").hide(),$("#show-grid").attr("checked",!1)}$canvaswrap.hasClass("mobile")&&!$canvaswrap.hasClass("tablet")&&$(".device-mobile").insertBefore($(".device-window"));$canvaswrap.hasClass("mobile-landscape")&&$(".device-mobile-landscp").insertBefore($(".device-window"));$canvaswrap.hasClass("tablet")&&$(".device-tablet").insertBefore($(".device-window"));
imgs=$(".device-wrap span");$("#canvas .wire").has(".headlinetxt").addClass("hastext");$(".textnew.ui-draggable").draggable("destroy");$canvaswrap.center();$("#canvas .locked").draggable("destroy").resizable("destroy");$("#sitemaplist").sortable({placeholder:"ui-state-highlight",containmentType:"div.sitemap",start:function(a,b){$(document).data("oldpos",b.item.index())},stop:function(a,b){var c=b.item.index(),g=b.item.attr("data-pageid");console.log($(document).data("oldpos")+" "+c);$(document).data("oldpos")>
c?$("#canvas #"+g).insertBefore($("#canvas .page:eq("+c+")")):$(document).data("oldpos")<c&&$("#canvas #"+g).insertAfter($("#canvas .page:eq("+c+")"))}}).disableSelection("#sitemaplist li")});$("#canvas .wire").not("#canvas .wire:has(.wirejr)").css({"border-style":"solid"});if($("#canvas .page").length)var multipaged=!0;
...
previewMode,$edittxt=$("#edittxt");$(".container").hasClass("preview")&&(previewMode=!0);
...
function edittext(a){if(!previewMode){a.stopPropagation();console.log($(this));activepage.boxer("destroy");a=$canvas.find(".selected .textme");saveState(a.closest(".wire"));a.parent().parent().parent().removeClass("selected").css("z-index",9999999);var b=a.html().replace(/<br>/gi,"\n"),c=$("<textarea class='editingtext' tabindex='-1' ></textarea>");c.val($("<div />").html(b).text());a.replaceWith(c);c.focus();c.bind("blur",textblur);c.autosize()}}
function focustext(){$(".latest .editingtext").focus()}function textblur(){previewMode||useboxer();var a=$canvas.find(".editingtext");a.parent().parent().parent().css("z-index","auto");var b=a.val().replace(/\n/gi,"<br>"),c=$("<div>");""===b&&(b="...");c.html(b).addClass("textme");a.replaceWith(c);a=c.css("height");c.closest(".wire").css({height:a,position:"absolute"});c.closest(".wirejr").draggable("destroy")}
function editableTextBlurred(){previewMode||useboxer();$(".textareanow").parent().removeClass("latest");var a=$(".textareanow").val().replace(/\n/gi,"<br>"),b=$("<div>");""===a&&(a="...");b.html(a);$(".textareanow").replaceWith(b);b.click(divClicked);$(".ok").hide();previewMode&&(saveComment(),b.parent().parent().draggable({cancel:".annotbox , .closeannot",cursor:"move"}).resizable({handles:"all"}))}$(".ok").click(editableTextBlurred);$canvas.bind("editThisText",".textme",edittext);
...
c=b.box.width(),20<b.box.width()||20<b.box.height()){$("#icons").show().siblings().hide();aspect_ratio($("#canvas .blank"));$("#annotate").show();$("#textinput").removeClass("lasticon");var d=b.box.position(),e=d.top,d=d.left,f=$("#canvas-wrap").position(),e=e+f.top-40,g=wirepanel.width(),c=d+c/2-g/2+f.left;0>=c&&(c=0);wirepanel.css({top:e,left:c});wirepanel.show()}else b.box.remove()}})}
wirepanel.delegate(".icon","hover",function(){var a=$(this);$canvas.find(".brandnew").length&&(type=a.attr("elementtype"),prepareWire())});wirepanel.delegate(".icon","click",function(){insertWire()});
...
function insertWire(){if($("#canvas .brandnew").length){if("annotate"!==type){var a=$(".brandnew");a.wrap("<div class='wire'></div>");var b=a.closest(".wire"),c=a.width(),d=a.height(),e=a.position(),f=e.top,e=e.left;$("#canvas .wirejr").not(".aa").each(function(){var a=parseInt($(this).css("zIndex"),10);a>index_highest&&(index_highest=a)});var g="w"+(new Date).getTime();saveState("",[g]);b.css({top:f,left:e,width:c+2*defbw,height:d+2*defbw}).attr("id",g);a.css("cssText","top: 0 !important;left: 0 !important;height:"+
d+"px;width:"+c+"px;border-width:"+defbw+"px;border-color:"+defstrcolor+";color:"+defstrcolor+";background:"+defbgcolor+";z-index:"+(index_highest+1)+";");a.is(".aa")&&a.css("z-index","5000").removeClass("wirejr");makeDrag(b)}else makeDrag($(".brandnew")),$(".brandnew").removeClass("wirejr").css("z-index",5E3);"headlinetxt"==type&&($(".brandnew").parent().addClass("hastext selected"),$(".textme.thelatest").trigger("editThisText"),$(".textnew.lastheadline").removeClass("lastheadline"));"annotate"==
type&&($(".editme").click(divClicked),$(".thelatest").click(),$(".closeannot").click(function(){$(this).parent().remove()}));$(".brandnew").removeClass("blank brandnew");wirePresent&&0<$(".wire").length&&(showSaveButton(),wirePresent=!1);wirepanel.fadeOut("fast")}}var type;$(".headlinetxt").click(function(){$(this).draggable({disabled:!1})}).dblclick(function(){$(this).draggable({disabled:!0})});
...
var d=[];c.each(function(a){var b="w"+(new Date).getTime()+a;$(this).attr("id",b);d[a]=b});historystate.shift();saveState("",d);makeDrag(c);$(".wire").draggable("option","helper","none");$("#canvas").droppable("destroy")}}))});a.filter(".locked").click(function(a){0==$("#canvas .brandnew").length&&0===$("#canvas .selected:not(.locked)").length&&($(this).addClass("selected"),$("#lock").addClass("pressed"))});var b;a.not(".groupmember , .wirejr , .locked").draggable({snap:".col ",start:function(a,e){var f=
$(this);$canvaswrap.find(".brandnew").remove();f.hasClass("selected")?saveState():saveState(f);c=parseInt($(this).css("borderLeftWidth"),10);$(".selected.locked").removeClass("selected");e.helper.data("draggableXY.originalPosition",e.position||{top:0,left:0});e.helper.data("draggableXY.newDrag",!0);!a.shiftKey&&2>$("#canvas .selected:not(.locked)").length&&$(".selected").removeClass("selected");b=$("#canvas .selected:not(.locked)");$(".dim").html("<b>W</b>: "+$(this).outerWidth()+"px, <b>H</b>: "+
$(this).outerHeight()+"px");wirepanel.hide();b.children().removeClass("selected");posTopArray=[];posLeftArray=[];$(this).hasClass("selected")&&b.each(function(a){thiscsstop=$(this).css("top");"auto"==thiscsstop&&(thiscsstop=0);thiscssleft=$(this).css("left");"auto"==thiscssleft&&(thiscssleft=0);posTopArray[a]=parseInt(thiscsstop,10);posLeftArray[a]=parseInt(thiscssleft,10)});begintop=$(this).offset().top;beginleft=$(this).offset().left;0==gridsize&&"col"==$("select#grid-type").val()&&!0==$("#snap-to-grid").attr("checked")?
guides=multipaged?$.map($("#canvas .page:visible .wire, #canvas , #gridcol .col").not(this),computeGuidesForElement):$.map($("#canvas .wire, #canvas , #gridcol .col").not(this),computeGuidesForElement):0==gridsize&&(guides=multipaged?$.map($("#canvas .page:visible .wire, #canvas").not(this),computeGuidesForElement):$.map($("#canvas .wire, #canvas").not(this),computeGuidesForElement));innerOffsetX=a.pageX-this.offsetLeft;innerOffsetY=a.pageY-this.offsetTop},drag:function(a,c){if(a.shiftKey){var f=
c.helper.data("draggableXY.originalPosition"),g=Math.abs(f.left-c.position.left),f=Math.abs(f.top-c.position.top),h=c.helper.data("draggableXY.newDrag");c.helper.data("draggableXY.newDrag",!1);g=h?Math.max(g,f)===g:c.helper.data("draggableXY.xMax");c.helper.data("draggableXY.xMax",g);g&&$(this).draggable("option","axis","x");g||$(this).draggable("option","axis","y")}if($(this).hasClass("selected")){var k=$(this).offset().left-beginleft,x=$(this).offset().top-begintop;b.each(function(a){$(this).css("top",
posTopArray[a]+x);$(this).css("left",posLeftArray[a]+k)})}if(0==gridsize){$("#canvas").offset();var v,u,s=MIN_DISTANCE+1,q=MIN_DISTANCE+1,t,m,f=$(this),n=a.pageY-innerOffsetY,p=a.pageX-innerOffsetX,l=f.outerWidth()-1,r=f.outerHeight()-1,w;$.each(guides,function(a,b){"h"==b.type&&((w=Math.abs(n-b.y))<q&&(q=w,u=b,m=0),(w=Math.abs(n-b.y+r))<q&&(q=w,u=b,m=r));"v"==b.type&&((w=Math.abs(p-b.x))<s&&(s=w,v=b,t=0),(w=Math.abs(p-b.x+l))<s&&(s=w,v=b,t=l))});if(!a.shiftKey||a.shiftKey&&!g)q<=MIN_DISTANCE?($("#guide-h").css("top",
u.y).show(),c.position.top=u.y-m):($("#guide-h").hide(),c.position.top=n+1);if(!a.shiftKey||a.shiftKey&&g)s<=MIN_DISTANCE?($("#guide-v").css("left",v.x).show(),c.position.left=v.x-t):($("#guide-v").hide(),c.position.left=p+1)}if(0<gridsize){var g=c.position.left,f=c.position.top,l=$(this).outerWidth()-1,r=$(this).outerHeight()-1,h=c.position.left+l,y=c.position.top+r;snappedleft=Math.round(g/gridsize)*gridsize;snappedright=Math.round(h/gridsize)*gridsize;snappedtop=Math.round(f/gridsize)*gridsize;
snappedbottom=Math.round(y/gridsize)*gridsize;g=Math.abs(snappedleft-c.position.left);f=Math.abs(snappedright-c.position.left-l);h=Math.abs(snappedtop-c.position.top);y=Math.abs(snappedbottom-c.position.top-r);c.position.left=g<f?snappedleft:snappedright-l;c.position.top=h<y?snappedtop:snappedbottom-r}},stop:function(a,b){$("#guide-v, #guide-h").hide();$(this).hasClass("altdrag")?$(".altdrag").removeClass("selected altdrag"):$(this).addClass("selected");$(this).draggable("option","axis","none")}});
var c=2;a.not(".groupmember , .wirejr , .locked").resizable({maxWidth:cwidth+1,autoHide:!1,start:function(a,b){var f=multipaged?$("#canvas .page:visible .selected"):$("#canvas .selected");$("#guide-v , #guide-h").hide();handle=$(this).data("resizable").axis;saveState($(this));c=parseInt(f.css("borderLeftWidth"),10);wirepanel.hide();$(this).removeClass("selected");0==gridsize&&"col"==$("select#grid-type").val()&&!0==$("#snap-to-grid").attr("checked")?guides=multipaged?$.map($(" #canvas .page:visible .wire, #canvas , #gridcol .col").not(this),
computeGuidesForElement):$.map($("#canvas .wire, #canvas , #gridcol .col").not(this),computeGuidesForElement):0==gridsize&&(guides=multipaged?$.map($("#canvas .page:visible .wire, #canvas").not(this),computeGuidesForElement):$.map($("#canvas .wire, #canvas").not(this),computeGuidesForElement))},resize:function(a,b){if(0==gridsize){var f,g,h=MIN_DISTANCE+1,k=MIN_DISTANCE+1,x,v,u=b.position.top,s=b.position.left,q=b.size.width-1,t=b.size.height-1,m;$.each(guides,function(a,b){"h"==b.type&&((m=u-b.y)<
k&&(m=u-b.y)>-1*k&&"s"!==handle&&"se"!==handle&&"sw"!==handle&&(k=m,g=b,v=0),(m=u-b.y+t)<k&&(m=u-b.y+t)>-1*k&&"n"!==handle&&"ne"!==handle&&"nw"!==handle&&(k=m,g=b,v=t));"v"==b.type&&((m=s-b.x)<h&&(m=s-b.x)>-1*h&&"e"!==handle&&"ne"!==handle&&"se"!==handle&&(h=m,f=b,x=0),(m=s-b.x+q)<h&&(m=s-b.x+q)>-1*h&&"w"!==handle&&"nw"!==handle&&"sw"!==handle&&(h=m,f=b,x=q))});k<=MIN_DISTANCE&&("n"==handle||"ne"==handle||"nw"==handle)?($("#guide-h").css("top",g.y).show(),b.position.top=g.y-v,b.size.height+=k):k<=
MIN_DISTANCE&&("s"==handle||"sw"==handle||"se"==handle)?($("#guide-h").css("top",g.y).show(),b.size.height-=k+2*c):h<=MIN_DISTANCE&&("w"==handle||"sw"==handle||"nw"==handle)?($("#guide-v").css("left",f.x).show(),b.position.left=f.x-x,b.size.width+=h+1):h<=MIN_DISTANCE&&("e"==handle||"se"==handle||"ne"==handle)?($("#guide-v").css("left",f.x).show(),b.size.width-=h+2*c):$("#guide-v , #guide-h").hide()}if($(".ui-resizable-resizing").hasClass("svginside")){var n=b.size.width;$(".ui-resizable-resizing svg ").css("width",
n);var p=b.size.height;$(".ui-resizable-resizing svg ").css("height",p)}if($(".ui-resizable-resizing").has("hasSVG")){var n=$(".ui-resizable-resizing").width(),p=$(".ui-resizable-resizing").height(),l=$(".ui-resizable-resizing .line2").attr("stroke-width");$(".ui-resizable-resizing .line1").attr("x2",n-2*l);$(".ui-resizable-resizing .line1").attr("y1",p-2*l);$(".ui-resizable-resizing .line2").attr("x2",n-2*l);$(".ui-resizable-resizing .line2").attr("y2",p-2*l);$(".ui-resizable-resizing svg").attr("height",
p-2*l);$(".ui-resizable-resizing svg").attr("width",n-2*l)}if(0<gridsize){var n=b.position.left,p=b.position.top,q=b.size.width-1,t=b.size.height-1,l=b.position.left+q,r=b.position.top+t;snappedleft=Math.round(n/gridsize)*gridsize;snappedright=Math.round(l/gridsize)*gridsize;snappedtop=Math.round(p/gridsize)*gridsize;snappedbottom=Math.round(r/gridsize)*gridsize;Math.abs(snappedleft-b.position.left);Math.abs(snappedright-b.position.left-q);Math.abs(snappedtop-b.position.top);Math.abs(snappedbottom-
b.position.top-t);"se"==handle&&(b.size.width=snappedright-n-2*c+1,b.size.height=snappedbottom-p-2*c+1);"e"==handle&&(b.size.width=snappedright-n-2*c+1);"s"==handle&&(b.size.height=snappedbottom-p-2*c+1);"w"==handle&&(b.size.width=l-snappedleft-2*c+1,b.position.left=snappedleft);"n"==handle&&(b.size.height=r-snappedtop-2*c+1,b.position.top=snappedtop);"nw"==handle&&(b.size.height=r-snappedtop-2*c+1,b.position.top=snappedtop,b.size.width=l-snappedleft-2*c+1,b.position.left=snappedleft);"ne"==handle&&
(b.size.height=r-snappedtop-2*c+1,b.position.top=snappedtop,b.size.width=snappedright-n-2*c+1);"sw"==handle&&(b.size.width=l-snappedleft-2*c+1,b.position.left=snappedleft,b.size.height=snappedbottom-p-2*c+1)}$(".dim").html("<b>W</b>: "+(b.size.width+2*c-1)+"px, <b>H</b>: "+(b.size.height+2*c-1)+"px")},stop:function(a,b){$("#guide-v , #guide-h").hide();$(this).addClass("selected");if($(".ui-resizable-resizing").hasClass("svginside")){var c=b.size.width;$(".ui-resizable-resizing svg ").css("width",
c);c=b.size.height;$(".ui-resizable-resizing svg ").css("height",c)}},snap:" .col ",handles:"all",autoHide:!0});a.each(function(){$(this).resizable("option","alsoResize",$(this).find(".wirejr"))});a.mouseover(function(b){var c;c=multipaged?$("#canvas .page:visible .selected"):$("#canvas .selected");$.each(a,function(){$(this).bind("contextmenu",function(a){1==c.length&&c.removeClass("selected");$(this).addClass("selected")})});isover=!0;var f=$(this),g=!1;$(document).bind("keydown",function(a){g||
(g=!0,a.shiftKey&&!0==isover&&f.draggable("disable"))})}).mouseout(function(a){isover=!1;$(document).unbind("keydown");a=$(this);a.hasClass("ui-draggable-disabled")&&a.draggable("enable")});$(".grouped").length&&$(".grouped").resizable("destroy")}var isover=!1;
...
$(".canvassize").html("<b>canvas</b>: "+$("#canvas").width()+"/"+$("#canvas").height()+"px"));$("#canvas").css("width","100%");$("#canvas").css("height","100%")},stop:function(a,b){supercanvas.css("height","100%");cwidth=b.size.width;$canvaswrap.hasClass("mobile-landscape")&&(cwidth=480);$("#canvas .wire").resizable("option","maxWidth",cwidth);$(this).center();updateCanvasInput()},alsoResize:".container",handles:"se"}).draggable({handle:"#browser",cursor:"move",containment:".container",stop:function(a){80>
parseInt($(this).css("top"),10)&&$(this).transition({top:"110px"});offset_c=$("#canvas").offset()}});$canvaswrap.find(".ui-resizable-se").addClass("icon-resize-full")}MakeCanvasDrag();$("#del").click(function(){delnow()});function delnow(){var a=$canvas.find(".selected");saveState(a);a.remove()}$("#edit").click(function(){$("#icons").show().siblings().hide();$('.icon[elementtype="headlinetxt"]').hide();positionwirepanel()});
function positionwirepanel(){offset_c=$("#canvas-wrap").position();var a=$("#canvas div.selected");a.height();var b=a.width(),c=a.position(),a=c.left,c=c.top+offset_c.top-45,d=wirepanel.width(),b=a+b/2-d/2+offset_c.left;0>=b&&(b=0);wirepanel.css({top:c,left:b}).show()}function editme(a){a.attr("data-changed","true");a.length&&(render_edit_menu(),positionwirepanel())}$edittxt.click(function(){$canvas.find(".selected div.textme").trigger("editThisText");wirepanel.hide()});var copied;posTopArray=[];
posLeftArray=[];$("#copy").click(function(){copynow()});$("#cut").click(function(){cutnow()});key("ctrl+c, command+c",function(){copynow()});key("ctrl+x, command+x",function(){cutnow()});
...
function cutnow(){var a=multipaged?$("#canvas .page:visible .selected"):$("#canvas .selected");a.addClass("copying").resizable("destroy").draggable("destroy");var b="";a.each(function(a){b+=$(this).outerHTML()});localStorage.setItem("clipboard",b);$(".selected").remove();$(".copied").fadeIn().delay(600).fadeOut()}key("del",function(){delnow()});$("#paste").click(function(){paste()});key("ctrl+v, command+v",function(){paste()});
function paste(){$canvas.find(".selected").removeClass("selected");var a=localStorage.getItem("clipboard");activepage.append(a);a=$canvas.find(".copying");makeDrag(a);$canvas.find(".copying.locked").length&&$canvas.find(".copying.locked").click(function(){$(this).addClass("selected");$("#lock").addClass("pressed")});var b=[];$.each(a,function(a){var d="w"+(new Date).getTime()+a;b[a]=d;$(this).attr("id",d).removeClass("copying")});saveState("",b)}
$(".colorpicker").click(function(){var a=$(this).css("background-color");changeBackgroundColor(a)});$(".strokeweight").click(function(){changeBorderWidth($(this).attr("stroke"))});$.fn.getId=function(){if($(this).attr("id"))return $(this).attr("id");var a="w"+(new Date).getTime();$(this).attr("id",a);return a};
...
function computeGuidesForElement(a){var b=$(a);b.is(".col")?(a=b.position(),ww=$("#canvas").width(),a.left+=.01*ww):a=b.position();if(b.is(".col"))var c=b.width(),b=b.height()-1;else c=b.outerWidth()-1,b=b.outerHeight()-1;return[{type:"h",x:a.left,y:a.top},{type:"h",x:a.left,y:a.top+b},{type:"v",x:a.left,y:a.top},{type:"v",x:a.left+c,y:a.top}]}function computeColForElement(a){var b=$(a);a=b.position();ww=$("#canvas").width();a.left+=.01*ww;b=b.width();return[a.left+1,a.left+b]}
function aspect_ratio(a){var b=a.width();a=a.height();b/=a;.3<=b&&8>=b&&($(".icon.rect").show(),$(".icon:not(.rect)").hide());.3>b&&($(".icon.vertical").show(),$(".icon:not(.vertical)").hide());8<b&&($(".icon.horizontal").show(),$(".icon:not(.horizontal)").hide());$("#annotate").hide();$("#textinput").addClass("lasticon")}
...
function updategrid(a){for(var b=0;5>b;b++){var c="#grid-pattern .lineV:eq("+[b]+")";$("#grid-pattern .lineH:eq("+[b]+")").attr({y1:[b+1]*a,y2:[b+1]*a,x2:5*a});$(c).attr({x1:[b+1]*a,x2:[b+1]*a,y2:5*a});$(".svgdot").attr({cx:a-.5,cy:a-.5})}$("#grid-pattern").attr({width:5*a,height:5*a});$("#dot-pattern").attr({width:a,height:a})}$("#grid-slider").slider({min:10,max:50,value:20,slide:function(a,b){$(".grid-size-val").text(b.value)},stop:function(a,b){updategrid(b.value);turngridon()}});
...
function saveState(a,b,c){var d=[],e=!1;"undefined"===typeof a&&(a=$canvas.find(".selected"));b?d={deleteid:b}:a.hasClass("grouped")?e=!0:$.each(a,function(a){var b=$(this),c=b.getId();d[a]={html:b.find(".wirejr").outerHTML(),css:{top:b.css("top"),left:b.css("left"),width:b.css("width"),height:b.css("height")},id:c,orgclass:b.attr("class"),parent:b.parent().attr("id")}});a=c?future:historystate;e||a.unshift(d);3<a.length&&a.pop();$("#undo").hasClass("inactive")&&$("#undo").removeClass("inactive")}
key("ctrl+z, command+z",function(a){a.preventDefault();restoreState(!0)});$("#undo").click(function(){restoreState(!0)});key("ctrl+y, command+y",function(a){a.preventDefault();restoreState(!1)});$("#redo").click(function(){restoreState(!1)});
function restoreState(a){var b;b=a?historystate:future;if(0<b.length){var c=b[0],d=$();if(c.deleteid)deletePasted(c,a);else{$.each(c,function(a){d=d.add($("#"+c[a].id))});$("#"+c[0].id).length&&saveState(d,!1,a);var e,f=!1,g=[];for(e=0;e<c.length;++e)$("#"+c[e].id).length?$("#"+c[e].id).resizable("destroy").draggable("destroy").css(c[e].css).html(c[e].html):($("<div/>").appendTo($("#"+c[e].parent)).addClass(c[e].orgclass).removeClass("ui-draggable ui-resizable ui-resizable-autohide").html(c[e].html).attr("id",
c[e].id).css(c[e].css),d=d.add($("#"+c[e].id)),g[e]=c[e].id,f=!0);f&&saveState("",g,a);makeDrag(d)}b.shift();$("#redo").hasClass("inactive")&&$("#redo").removeClass("inactive")}else $("#undo").addClass("inactive")}function deletePasted(a,b){var c=$();$.each(a.deleteid,function(a){c=c.add($("#"+this))});saveState(c,!1,b);c.remove()}
...
f+' style="position:absolute;top:'+b+"px; left:"+c+"px;width:"+(d-c+2)+"px;height:"+(e-b+2)+'px;"/>');a.parent().parent().addClass("selected");a=$("#"+f);makeDrag(a);a.resizable("destroy");$("#group").addClass("pressed")}}
...
jQuery.fn.htmlClean=function(){this.contents().filter(function(){if(3!=this.nodeType)return $(this).htmlClean(),!1;this.textContent=$.trim(this.textContent);return!/\S/.test(this.nodeValue)}).remove();return this};$canvas.bind("click",".grouped .wirejr",function(a){$(a.target).closest(".grouped").addClass("selected")});